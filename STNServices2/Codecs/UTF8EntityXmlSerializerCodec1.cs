...
namespace STNServices2.Codecs.xml
{
    [MediaType("application/xml;q=0.4", ".xml")]
    public class UTF8EntityXmlSerializerCodec1 : UTF8XmlCodec1
    {
        #region Methods
        ...
        protected XmlAttributeOverrides OverrideReferenceAttributes(Type entityType)
        {
            ...
            List<string> properties = entityType.GetProperties()
                    .Where(e => e.Name.Contains("Reference") || (!e.PropertyType.IsPrimitive && !e.PropertyType.Equals(typeof(string)) && !e.PropertyType.Equals(typeof(DateTime))) && 
                    !e.Name.Contains("Links") && !String.Equals(e.PropertyType.Name, "Nullable`1")).Select(e => e.Name).ToList();

            // assign XmlAttribute to override those fields with XmlIgnoreAttribute    
            foreach (string propertyName in properties)
            {
                xmlAttribute = new XmlAttributes { XmlIgnore = true };
