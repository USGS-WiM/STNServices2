...
namespace STNServices2.Handlers
{
    public class DeploymentPriorityHandler : STNHandlerBase
    {
        ...
        public OperationResult POST(deployment_priority anEntity)
        {
            try
            {
                if (string.IsNullOrEmpty(anEntity.priority_name))
        ...
        public OperationResult Put(Int32 entityId, deployment_priority anEntity)
        {
            try
            {
                if (entityId <=0 || string.IsNullOrEmpty(anEntity.priority_name))
                    throw new BadRequestException("Invalid input parameters");
                using (EasySecureString securedPassword = GetSecuredPassword())
                {
                    using (STNAgent sa = new STNAgent(username, securedPassword))
                    {
                        anEntity = sa.Update<deployment_priority>(entityId, anEntity);
                        sm(sa.Messages);
                    }//end using
                }//end using
