    ...
    public class SensorBrandHandler : STNHandlerBase
    {
        ...
        public OperationResult GetInstrumentSensorBrand(Int32 instrumentId)
        {
            ...
            try
            {
                ...
                using (STNAgent sa = new STNAgent())
                {
                    anEntity = sa.Select<instrument>().Include(i=>i.sensor_brand).FirstOrDefault(i => i.instrument_id == instrumentId).sensor_brand;
                    if (anEntity == null) throw new NotFoundRequestException();
                    sm(sa.Messages);
                }//end using
        ...
        public OperationResult Put(Int32 entityId, sensor_brand anEntity)
        {
            try
            {
                ...
                using (EasySecureString securedPassword = GetSecuredPassword())
                {
                    using (STNAgent sa = new STNAgent(username, securedPassword))
                    {
                        anEntity = sa.Update<sensor_brand>(entityId, anEntity);
                        sm(sa.Messages);
                    }//end using
                }//end using
