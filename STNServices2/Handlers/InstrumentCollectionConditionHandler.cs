    ...
    public class InstrumentCollectionConditionHandler : STNHandlerBase
    {
        ...
        public OperationResult GetInstrumentCondition(Int32 instrumentId)
        {
            ...
            try
            {
                ...
                using (STNAgent sa = new STNAgent())
                {
                    anEntity = sa.Select<instrument>().Include(i=>i.instr_collection_conditions).FirstOrDefault(i => i.instrument_id == instrumentId).instr_collection_conditions;
                    if (anEntity == null) throw new NotFoundRequestException(); 
                    sm(sa.Messages);
                }//end using
        ...
        public OperationResult Put(Int32 entityId, instr_collection_conditions anEntity)
        {
            try
            {
                ...
                using (EasySecureString securedPassword = GetSecuredPassword())
                {
                    using (STNAgent sa = new STNAgent(username, securedPassword))
                    {
                        anEntity = sa.Update<instr_collection_conditions>(entityId, anEntity);
                        sm(sa.Messages);
                    }//end using
                }//end using
